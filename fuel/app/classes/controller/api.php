...
class PDF extends FPDF {
  ...
  function BasicTable($pdf, $header, $data, $filename)
  {
    ...
    $oldX = $this->GetX();
    $oldY = $this->GetY();
    $this->SetXY($X, $this->GetY() + 11);
    $this->Write(10, $height . ' mm');
...
function getPdf($firstId, $secondId, $thirdId, $output = 'D') {
  ...
  foreach (array('first', 'second', 'third') as $key => $elem) {
    if ($result[$elem]) {
      $height = $result[$elem]->image_size_y;
      if ($elem == 'third' && $result['second']->connection == 'DOWN') {
        $height = 0;
      }

      $data[] = array($key + 1 .'.', $result[$elem]->name, $result[$elem]->weight . ' kg', $height);
    }
  }
